...
const ProjectAssignmentPanel = ({ project, onAssigned, onClose }) => {
  ...
  const handleAssign = async (userId) => {
    ...
    try {
      ...
      if (response.status === 200) {
        onAssigned(response.data.project);
        onClose();
      }
    } catch (error) {
      setMessage(error.response?.data?.message || "Failed to assign project.");
    } finally {
      setAssigningId("");
    }
  };
  ...
  return (
    <div className="assign-panel" ref={panelRef}>
      <label className="assign-panel-label" htmlFor={`project-assign-search-${project._id}`}>
        Search members by name
      </label>
      <input
        className="assign-search-input"
        id={`project-assign-search-${project._id}`}
